...
select sum(PRICE*QUANTITY) as total_sale_amount, PURCHASE_DATE as "YEAR" from ecommerce group by PURCHASE_DATE;  /* Time Elapsed:  00:00:00:17*/
...
select count(customerno) as count,country from ECOMMERCE group by country;  /* Time Elapsed:  00:00:00:13*/

explain plan set statement_id='13' for select count(customerno) as count,country from ECOMMERCE group by country;   --Explained--
...
--CREATE BIPMAP INDEX--
/*create bitmap index prname_date_idx on ecommerce(productname,extract(year from TO_DATE ("PURCHASE_DATE",'YYYY-MM-DD')),extract(month from TO_DATE ("PURCHASE_DATE",'YYYY-MM-DD')));*/
...
-- QUERY1--
explain plan set  statement_id='15' for select sum(quantity) as products_sold from ecommerce where (extract (year from to_date(Purchase_Date,'YYYY-MM-DD')))=2019 
...
--QUERY5--
explain plan set  statement_id='19' for SELECT DISTINCT PRODUCTNAME AS UNIQUE_PRODUCTS,EXTRACT(year FROM TO_DATE(Purchase_Date,'YYYY-MM-DD')) as YEAR_FIELD 
