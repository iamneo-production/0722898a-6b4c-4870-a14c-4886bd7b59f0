...
create index index_on_year_and_mon on ecommerce((extract (year from to_date(PURCHASE_DATE,'YYYY-MM-DD'))),(extract (month from to_date(PURCHASE_DATE,'YYYY-MM-DD'))));
...
select sum(quantity) as products_sold from ecommerce where (extract (year from to_date(PURCHASE_DATE,'YYYY-MM-DD')))=2019 and (extract (month from to_date(PURCHASE_DATE,'YYYY-MM-DD')))=2 ;
...
select sum(price*quantity) as total_sale,extract (year from to_date(PURCHASE_DATE,'YYYY-MM-DD')) as year_of_sale from ecommerce group by
